...
Run from package root (directory containing `newsai`):
  streamlit run newsai/dashboard/app.py
"""
...
from newsai.db.connection import get_session_factory
from newsai.db.models import Article, LabeledArticle
...
if not rows:
    st.info("No rows yet. Run `python -m newsai.jobs.run_once` after DB setup.")
else:
    import pandas as pd
