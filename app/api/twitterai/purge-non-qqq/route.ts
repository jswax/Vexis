...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${TWITTER_AI_URL}/api/twitter/purge-non-qqq`, {
      method: "POST",
      headers,
      body: JSON.stringify(body),
    });
    const data = await res.json().catch(() => null);
    console.log("[twitterai] purge-non-qqq response", res.status, data);
    return NextResponse.json(data, { status: res.status });
  } catch (err) {
    console.error("[twitterai] purge-non-qqq proxy error", err);
